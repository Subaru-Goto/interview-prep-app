...
- Three **interviewer** system-prompt variants — zero-shot, few-shot, role-play persona — held in a **prompt registry**. (Originally scoped as five, including structured-rubric and chain-of-thought; both dropped deliberately — see `docs/issues/010-prompt-bakeoff-writeup.md` for why.) A controlled bake-off across 2–3 fixed `(CV, JD)` fixtures selects the winner; each technique is written up for submission. Chain-of-thought is instead demonstrated by the **planner** (its own dedicated CoT pass, unrelated to the interviewer bake-off); judge (structured output) demonstrates a further technique naturally.
